...
def main() -> None:
    ...
    baseline_counts["weighted_tractor_count"] = pd.to_numeric(
        baseline_counts["weighted_tractor_count"], errors="coerce"
    ).fillna(0)
    ...
    for scenario_name in scenario_names:
        target_counts = (
            scenario_tier.loc[
                scenario_tier["scenario_name"] == scenario_name,
                ["tier_assigned", "weighted_tractor_count"]
            ]
            .copy()
            .set_index("tier_assigned")
            .reindex(TIERS)
        ...
        target_counts["weighted_tractor_count"] = pd.to_numeric(
            target_counts["weighted_tractor_count"], errors="coerce"
        ).fillna(0)
        ...
        if "regional_diesel_ml" not in target_reg.columns:
            target_reg["regional_diesel_ml"] = pd.to_numeric(
                target_reg["annual_diesel_litres"], errors="coerce"
            ) / 1_000_000
        if "regional_scr_diesel_ml" not in target_reg.columns:
            target_reg["regional_scr_diesel_ml"] = pd.to_numeric(
                target_reg["scr_diesel_litres"], errors="coerce"
            ) / 1_000_000
        if "def_central_ml" not in target_reg.columns:
            target_reg["def_central_ml"] = (
                pd.to_numeric(target_reg["scr_diesel_litres"], errors="coerce") * 0.04
            ) / 1_000_000
        ...
        target_reg = target_reg[[
            ...
            "def_central_ml": "regional_def_central_ml"
        })
        ...
        for col in [
            ...
            "regional_def_central_ml_target"
        ]:
        ...
        for year in years:
            ...
            reg_tmp["regional_def_central_ml"] = reg_tmp["regional_def_central_ml_base"] + alpha * (
                reg_tmp["regional_def_central_ml_target"] - reg_tmp["regional_def_central_ml_base"]
            )
